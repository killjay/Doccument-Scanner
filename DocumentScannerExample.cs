...
namespace OpenCVForUnityExample
{
    ...
    public class DocumentScannerExample : MonoBehaviour
    {
        ...
        // need to use it to find  4 corners
        ...
        //---------------------------------------------------------------------------------------------------------
        public void SaveOutputDisplayAreaAsPNG()
        {
            
            string filePath = Application.persistentDataPath + "output_display_area.png";
            // Ensure outputDisplayAreaMat is not null and not empty
            if (outputDisplayAreaMat != null && !outputDisplayAreaMat.empty())
            {
                // Define the file path for saving the PNG
                

                // Write the outputDisplayAreaMat as a PNG file
                Imgcodecs.imwrite(filePath, outputDisplayAreaMat);

                // Log the path to the saved PNG file
                Debug.Log("Output display area saved as PNG: " + filePath);
            }
            else
            {
                // Log an error if outputDisplayAreaMat is null or empty
                Debug.LogError("Output display area is null or empty. Cannot save as PNG.");
            }
            foreach(GameObject obj in objectsCam)
            {
                obj.SetActive(false);
            }
